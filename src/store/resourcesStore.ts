...
export const useResourcesStore = create<ResourcesStoreState>()((set, get) => ({
  ...
  upsertResources: (resources) => set((state) => {
    ...
    return next.length === state.resources.length ? persist(next) : { ...persist(next), past: [...state.past, structuredClone(state.resources)].slice(-50), future: [] };
  }),
  ...
  replaceResources: (resources) => set((state) => ({ ...persist(resources), past: [...state.past, structuredClone(state.resources)].slice(-50), future: [] })),
